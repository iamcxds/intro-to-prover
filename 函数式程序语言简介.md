...
```elm
type UnionAB -- 把类型A，B并起来的类型，由于elm没有函子，我们只能定义这样固定的类型
  = Incl1 A
  | Incl2 B -- |用这个符号来表示并列
  | Incl3 C -- 还可以继续这样并上更多的类型
  | Incl4 D E --甚至可以连跟两个类型，这样类似并上了 D×E

Incl1 : A -> UnionAB -- Incl1 既是这样一个函数，又是一个记号
Incl1 a : UnionAB -- a:A，UnionAB的项都可以写成这样的形式

--当然乘积类型也是有的
--二元乘积类型
(a,b) : ( A , B )--elm里用这种括号来代表二元乘积类型
--多元乘积类型就是record了
type alias ColorPoint3D =
  { x : Int
  , y : Int
  , z : Int
  , color : Color
...
我们注意到在定义中我们会在每个要并列的类型前加上一个符号，这个符号既代表无交并中的 canonical inclusions (如上面交换图中的$i_1, i_2$)，另一方面这也是并类型的项构造器。
...
```elm
() : () -- elm中()即表示unit type 又表示它唯一的项，因为elm的类型和项界限分明所以可以这样表示

contraA : A -> ()  -- A 到 unit 的唯一函数
contraA a = ()

isoA : A -> () -> A -- 值得注意的是类型A同构于函数类型 ()->A，结合 curry 化，我们可以说所有类型都是(一元)函数类型
isoA a _ = a -- 当某个变量不影响结果时，我们可以用_来代替它。

type Msg  --之前使用的 Msg
  = Increment -- 可以是当做 Increment () 的简写
  | Decrement --这实际上是定义了一个二元集合

Increment : Msg
```

随之而来的问题，我们如何定义并类型的函数。事实上再次查看泛性质的交换图我们可以知道$\rm{Hom}(A\sqcup B,C)=\rm{Hom}(A,C)\times \rm{Hom}(B,C)$，所以我们实际上是需要对每个部分都定义函数了。所以为了能实现这件事，为什么需要模式匹配，某种意义上是 `if` 或者 `switch` 结构的升级版，而且不仅仅是匹配值，匹配的是模式

```elm
fun : UnionAB -> R
fun uAB =
  case uAB of --对于uAB分情况讨论
    Incl1 a -> ... --当项来自于A时的情况
    Incl2 b -> ...
...
```idris
get : (li : List A) -> Finset (length li) -> A -- 将第二个变量限制在比 length 更小的有限集合中
```
...
了解了这些内容，基本上可以算是 FP 入门了。凭借这些知识外加一些 elm 一些语法糖(比如如何使用或更新 record)，还有网页交互的 api 应该可以做出一些[简单的游戏](https://github.com/rofrol/elm-games)了(我一直认为做游戏是学习编程最好的方式)。我个人很推荐的一个[像素游戏引擎](https://github.com/Orasund/pixelengine)(我自己也是用它做的游戏)，并且还有很详尽的[教程展示](https://orasund.github.io/pixelengine/)。

# 函子(functor)，monad 与 type class
