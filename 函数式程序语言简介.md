...
有人可能会问，FP 可以“面向对象”吗，事实上面向对象的部分一部分思想源于数学的**抽象**(abstract)，而 type class 可以很好的得实现这件事。(而关于面向对象的另外一部分思想，CRUD数据则可以通过很多 FP 都有的 package [Lens](https://hackage.haskell.org/package/lens) 来实现)
